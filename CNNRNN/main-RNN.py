...
def build_model(input_shape):
    ...
    output_return = Dense(1, name='return')(x)
    output_volatility = Dense(1, name='volatility')(x)
    model = Model(inputs, [output_return, output_volatility])
...
def main():
    ...
    plt.suptitle('SSE Index Predictions in RNN Model', fontsize=16)
    plt.subplot(2, 1, 1)
    ...
    plt.tight_layout()
    plt.savefig('results/SSE-rnn-only-predictions-RNN.png')
    plt.show()
